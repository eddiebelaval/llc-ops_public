...
the user's unique insight: the corrections that make agents reliable are the same patterns that make human engineering teams reliable. Scope discipline, preflight checks, incremental verification, visual output confirmation. The difference is that agents can be reprogrammed with a file drop.
