...
Generate an interactive HTML visualization to explain a concept, system, architecture, codebase structure, or workflow visually. the user learns best through spatial, interactive visuals — not walls of text.
...
A clean, copy-paste-friendly list of ALL prompts from the assessment, numbered and labeled. Each prompt row has a checkbox. This is the "action sheet" — the user can work through them top to bottom. Checking a prompt here also checks the corresponding item in sections 2/3, and vice versa (two-way sync).
