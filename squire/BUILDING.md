...
Commands were extracted from repeated prompt patterns. `/ship` came from the 20+ times the user typed the same deploy sequence. `/visualize` came from wanting consistent HTML architecture diagrams. Each command crystallized a workflow that was being done manually into a reusable prompt file.
