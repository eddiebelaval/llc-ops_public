...
6. **Do NOT create a file.** This is ephemeral output. If the user wants it saved, he'll say so.

### Priority Override

If the user says "roadmap for [specific pillar]", zoom into that pillar's sub-items using SPEC capabilities as the granularity level.
...
2. **Never create persistent files.** These are ephemeral computed outputs. Present them in conversation. the user saves what he wants.
