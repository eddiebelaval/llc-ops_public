...
The Triad should never require manual maintenance. This document specifies how drift is detected proactively and how AskUserQuestion is used to keep the triangle current without the user having to remember anything.

## Core Principle

**Detect drift at natural workflow boundaries. Ask one focused question. Update the docs. Move on.**

the user should never see a stale VISION or SPEC. The system watches for signals and intervenes at the right moment — not in the middle of flow, but at natural breakpoints (session start, pre-ship, post-gate).
...
If drift is detected and the user says "skip" twice for the same project:

- **Third time:** Escalate tone slightly: "SPEC is [N] days stale with [M] unrecorded changes. This is starting to accumulate."
- **After that:** Stop asking. the user is aware. Only re-prompt if drift crosses 30 days or 20+ commits.
...
2. **Always provide a skip option.** the user is busy. The system works around his schedule.
...
[Wait for the user's response. If he says something shifted, switch to /reconcile vision mode.]
```
...
- **VISION.md Soul section.** This only changes during pivots. Never auto-detect or prompt for soul changes. If it needs to change, the user will know.
